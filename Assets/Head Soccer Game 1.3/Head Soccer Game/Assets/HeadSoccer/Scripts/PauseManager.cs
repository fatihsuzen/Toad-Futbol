...
public class PauseManager : MonoBehaviour {
	...
	void touchManager (){
		if(Input.GetMouseButtonDown(0)) {
			...
			if (Physics.Raycast(ray, out hitInfo)) {
				...
				switch(objectHitName) {
					...
					case "Btn-Restart":
						UnPauseGame();
                        GameController.gameTime = GameSystem.Instance.gameTime;
						Application.LoadLevel(Application.loadedLevelName);
						break;
	 ...
	 public void PauseGame (bool isShow = true)
    {
	    ...
	    if(pausePlane && isShow)
	    	pausePlane.SetActive(true);
	    currentStatus = Status.PAUSE;
	}


	public void UnPauseGame (bool isShow = true){
		print("Unpause");
	    isPaused = false;
